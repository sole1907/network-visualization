...
def network_graph(yearRange, AccountToSearch):
    ...
    G = nx.from_pandas_edgelist(df=edge1, source='Source', target='Target', edge_attr=[
    ...
    traceRecode = []  # contains edge_trace, node_trace, middle_node_trace
    ############################################################################################################################################################
    colors = list(Color('lightblue').range_to(
        Color('darkblue'), len(G.edges())))
